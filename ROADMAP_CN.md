...
- [x] 批处理直接读取 `PHAssetResource`，避免为每张照片构建完整 `PHLivePhoto` 预览。
- [x] 元数据写入不再二次编码 JPEG；正向 JPEG 保留原始压缩像素。
- [x] 流式拼接 JPEG 和视频，降低千张级任务的内存峰值。
- [x] 长时间批处理使用两路并发，减少 VideoToolbox 竞争和热降频。
- [x] 缓存清理仅删除 MoLive 自己的目录。
- [x] 将 Files 安全范围 URL 立即复制到应用缓存，避免延迟转换时权限失效。
- [x] 正确统计批处理失败数。
- [ ] 将转换状态限定在 `MainActor`，消除 Swift 6 并发警告。
- [ ] 完善批处理控制（已支持停止、继续未完成项和失败重试，待补充 iCloud 下载进度）。
